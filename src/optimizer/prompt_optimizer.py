...
class _DspyPromptOptimizer:
    ...
    def optimize_prompts(
        ...
        opt_limit = self._optimizer_cm.limit if self._optimizer_cm else None
        max_workers = min(len(prompts), opt_limit) if opt_limit else len(prompts)

        def _optimize(prompt: PromptLocation) -> Optional[PromptPatch]:
