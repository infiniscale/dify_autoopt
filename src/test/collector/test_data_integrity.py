...
class TestNumericalAccuracy:
    ...
    def test_statistics_calculation_accuracy(self):
        ...
        # 创建已知数据
        results = [
            create_test_result(execution_id="acc_1", execution_time=1.0, tokens_used=100, cost=0.01,
                               status=TestStatus.SUCCESS),
            create_test_result(execution_id="acc_2", execution_time=2.0, tokens_used=200, cost=0.02,
                               status=TestStatus.SUCCESS),
            create_test_result(execution_id="acc_3", execution_time=3.0, tokens_used=300, cost=0.03,
                               status=TestStatus.FAILED),
            create_test_result(execution_id="acc_4", execution_time=4.0, tokens_used=400, cost=0.04,
                               status=TestStatus.SUCCESS),
        ]
