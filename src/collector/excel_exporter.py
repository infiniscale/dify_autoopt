...
class ExcelExporter:
    ...
    def _create_overview_sheet(self, wb, metrics: PerformanceMetrics, classification: ClassificationResult):
        ...
        ws['A23'] = "优秀"
        ws[
            'B23'] = f"{classification.excellent_count} ({classification.grade_distribution.get(PerformanceGrade.EXCELLENT, 0):.2f}%)"

        ws['A24'] = "良好"
        ws[
            'B24'] = f"{classification.good_count} ({classification.grade_distribution.get(PerformanceGrade.GOOD, 0):.2f}%)"

        ws['A25'] = "一般"
        ws[
            'B25'] = f"{classification.fair_count} ({classification.grade_distribution.get(PerformanceGrade.FAIR, 0):.2f}%)"

        ws['A26'] = "较差"
        ws[
            'B26'] = f"{classification.poor_count} ({classification.grade_distribution.get(PerformanceGrade.POOR, 0):.2f}%)"
